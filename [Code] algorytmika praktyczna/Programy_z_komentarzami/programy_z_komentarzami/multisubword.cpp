...
// Zmienne p oraz k reprezentuja odpowiednio poczatek oraz koniec wyznaczonego slowa
	int p, k, n;
	int Find(SufTree<int>::SufV& v, int d) {
// d jest glebokoscia wezla v
		d += v.k - v.p;
		v.e = v.s;
// Zlicz liczbe sufiksow osiagalnych z wezla v
		FOREACH(it, v.sons) v.e += Find(*(it->ND), d);
// Jesli liczba sufiksow jest nie mniejsza od c, oraz glebokosc aktualnego wezla jest wieksza od dlugosci aktualnie znalezionego slowa, to zaktualizuj wynik
		if (v.e >= n && d > k-p) {
			k=v.k;
			p=v.k-d;
		}
...
// Wczytaj dane
	while(cin >> text >> n) {
// Wyznacz podslowo slowa text wystepujace n razy
