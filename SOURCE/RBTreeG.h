...
public:
    ...
    D* data;             ///< Dane
    int color;          ///R0 B1
    treeNode(): l(NULL), r(NULL),p(NULL),color(1) {data=new D;};
    treeNode(K k,D d) : l(NULL), r(NULL),p(NULL),color(1)
    {
        key=k;
        data=d;
    }
    treeNode(K k,const D& d, treeNode* a, treeNode* b, treeNode* c) : l(a), r(b),p(c)
    {
        data=d;
        key=k;
        color=1;
