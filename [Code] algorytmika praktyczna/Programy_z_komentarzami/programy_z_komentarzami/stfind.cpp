...
// Funkcja wylicza dla kazdego wezla drzewa sufiksowego liczbe osiagalnych wierzcholkow, ktore reprezentuja sufiks slowa
int STSuffixC(SufTree<int>::SufV& v) {
	v.e = v.s;
	FOREACH(it, v.sons) v.e += STSuffixC(*(it->second));
	return v.e;
}
// Funkcja wyznacza liczbe wystapieñ wzorca t w tekscie, dla ktorego zostalo skonstruowane drzewo sufiksowe st
int STFindCount(SufTree<int> &st, const char* t) {
	int tp = 0,x;
	VAR(v, &(st.root));
// Rozpoczynajac od korzenia drzewa przesuwaj sie po nim zgodnie z wczytywanymi literami wzorca
	while(t[tp]) {
		if (!(v = v->sons[t[tp]])) return 0;
		for(x=v->p;x<v->k && t[tp];) if (t[tp++] != st.text[x++]) return 0;
	}
// Jesli wzorzec wystepuje w tekscie, to liczba jego wystapieñ jest rowna liczbie sufiksow osiagalnych z aktualnego wezla drzewa
	return v->e;
}
int main() { 
	string text;
// Wczytaj tekst, w ktorym beda wyszukiwane wzorce
	cin >> text;
	cout << "Przeszukiwany tekst: " << text << endl;
// Skonstruuj drzewo sufiksowe
	SufTree<int> tree(text.c_str());
// Wyznacz liczbe osiagalnych wierzcholkow-sufiksow dla kazdego wierzcholka drzewa sufiksowego
	STSuffixC(tree.root);
// Dla poszczegolnych wzorcow wyznacz liczbe ich wystapieñ w tekscie
