...
int main() {
	int n;
	BigNum a;
// Wczytaj liczbe ogniw lañcucha
	cin >> n;
	int c[n+1];
// Wczytaj stany poszczegolnych ogniw
	FOR(x,1,n) cin >> c[x];
// Dla kolejnych ogniw, jesli ogniwo to jest zalozone na pret, to zwieksz liczbe ruchow, ktore trzeba wykonac, oraz zmieñ stan ogniwa poprzedzajacego aktualne
	FORD(x,n,1) {
		a*=2;
		if (c[x]) {
			a+=1;
