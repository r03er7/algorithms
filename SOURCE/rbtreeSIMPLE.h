...
//nil->p=nil;
//nil->r=nil;
treeNode* root=nil;//new treeNode(MP(100,"root"),NULL,NULL,NULL);
treeNode* teeSearch(treeNode* x,int k) {
    if(x==nil||k==x->data.first)return x;
    if(k<x->data.first)return teeSearch(x->l,k);
    else return teeSearch(x->l,k);
}
